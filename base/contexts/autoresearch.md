...
> "Python이 아닌 harness/program.md로 연구를 프로그래밍한다."

## 행동 지침
- `harness/program.md`가 유일한 법이다. 수정 가능 파일, 지표, 실행 명령이 모두 여기에 정의됨
...
- 아이디어가 고갈되면 harness/program.md를 다시 읽고, 실패 패턴을 분석하고, 더 급진적 변경을 시도한다
...
LOOP FOREVER:
  ...
  2. 가설 수립 — harness/program.md 목표 + 이전 결과 기반
...
- Read → harness/program.md, 결과 파일, 로그 확인
- Grep → 지표 추출, 에러 추적
...
## 활성화
`이 세션은 autoresearch 모드로 진행합니다` 또는 harness/contexts/autoresearch.md를 참조하세요.
