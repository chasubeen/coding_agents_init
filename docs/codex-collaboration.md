...
3개 이상 독립 파일에 걸친 대규모 작업을, 여러 Codex 인스턴스에 분배해 격리된 git worktree에서 동시에 구현. `harness/orchestrator/` Python 모듈이 세션·실행·머지를 관리. 이때 Codex는 `workspace-write`로 코드를 직접 수정한다.
...
- `harness/hooks/codex-crosscheck-reminder.sh` — 교차검증 제안 nudge 훅
- `.codex/config.toml` — Codex CLI 프로필 참조 템플릿
- `AGENTS.md` — Codex 관점의 운영 지침
- `harness/orchestrator/` — 병렬 멀티에이전트 모듈
